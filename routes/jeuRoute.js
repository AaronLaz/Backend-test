 ...
 * /jeu/allbyzone/{idFestival}/details
 */
 ...
 * /jeu/allbyzone/{id} 
 */
router.get('/allbyzone/:idFestival/:idZone',(req,res,next) => {
    const id = req.params['idZone'];
    const id2 = req.params['idFestival'];
    db.queryAllWhere3Ordered2('Jeu','JeuReserve','Zone','ZoneFestival','idJeu','idJeu','idZone','idZone','idZone','idZone',{"JeuReserve.idZone":id},{"idFestival":id2},'nomZone',function(result){
        res.status(200).send(result);
    });
});

/**
 * Affichage de la liste des jeux par zone pour le dernier festival
 * /jeu/last/allbyzone
 */
router.get('/last/allbyzone',(req,res,next) => {
    db.queryAllWhere3OrderedCondition('Jeu','JeuReserve','Zone','ZoneFestival','idJeu','idJeu','idZone','idZone','idZone','idZone','idFestival = (SELECT idFestival FROM Festival WHERE annee = (SELECT MAX(annee) FROM Festival))','nomZone',function(result){
        res.status(200).send(result);
    });
});

/**
 * Affichage de la liste des jeux d'une zone spécifique pour le dernier festival
 * /jeu/last/allbyzone/{idZone}
 */
router.get('/last/allbyzone/:idZone',(req,res,next) => {
    const id = req.params['idZone'];
    db.queryAllWhere3Ordered2Condition('Jeu','JeuReserve','Zone','ZoneFestival','idJeu','idJeu','idZone','idZone','idZone','idZone',{"ZoneFestival.idZone":id},'idFestival = (SELECT idFestival FROM Festival WHERE annee = (SELECT MAX(annee) FROM Festival))','nomZone',function(result){
        res.status(200).send(result);
    });
});
